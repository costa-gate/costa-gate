...
export const buildPatioPosition = (
  ...
  const normalizedPilha = String(pilha).trim().padStart(2, '0');
  const normalizedFila = String(fila).trim().padStart(2, '0');
  const normalizedAltura = String(altura).padStart(2, '0');

  return `P${normalizedPilha}-F${normalizedFila}-H${normalizedAltura}`;
};
...
export const getContainerByNumber = async (
  ...
  const normalizedNumber = normalizeContainerNumber(numeroContainer);

  const { data, error } = await supabase
    .from('containers_terminal')
    .select('*')
    .eq('numero_container', normalizedNumber)
    .neq('status', 'Saiu')
    .maybeSingle();
...
export const createContainerFromMovement = async (
  ...
  const numeroContainer = normalizeContainerNumber(
    input.numeroContainer,
  );

  if (!numeroContainer) {
    throw new Error('Informe o número do contêiner.');
  }
...
export const moveContainerToStack = async (
  input: MovimentarContainerInput,
): Promise<ContainerTerminal> => {
  const user = await ensureAuthenticated();
  const container = await getContainerByIdInternal(input.containerId);
  ...
  if (!input.pilha.trim()) {
    throw new Error('Informe a pilha.');
  }

  if (!input.fila.trim()) {
    throw new Error('Informe a fila.');
  }

  if (input.altura < 1 || input.altura > 6) {
    throw new Error('A altura deve estar entre 1 e 6.');
  }

  const pilha = input.pilha.trim().padStart(2, '0');
  const fila = input.fila.trim().padStart(2, '0');
  const posicao = buildPatioPosition(pilha, fila, input.altura);

  const { data, error } = await supabase
    .from('containers_terminal')
    .update({
      unidade: input.unidade,
      patio: input.patio?.trim() || null,
      pilha,
      fila,
      altura: input.altura,
      posicao,
  ...
  await registerContainerEvent({
    ...
    patioNovo: updatedContainer.patio ?? null,
...
export const scheduleContainerExit = async (
  ...
  if (container.status === 'Saiu') {
    throw new Error(
      'Não é possível programar um contêiner que já saiu.',
    );
  }
